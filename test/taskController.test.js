...
beforeAll(async () => {
  ...
  user1 = await prisma.user.create({
    data: { name: "Bob", email: "bob@sample.com", hashedPassword: "nonsense_hash" }
  });
  user2 = await prisma.user.create({
    data: { name: "Alice", email: "alice@sample.com", hashedPassword: "nonsense_hash" }
  });
});
...
describe("testing task creation", () => {
  ...
  it("15. You can't create a task with a bogus user id.", async () => {
    ...
    try {
      ...
      expect(e.name).toMatch(/PrismaClientKnownRequestError|TypeError/);
    }
  });
